...
def param_generator(func_or_scope=None, scope: str = "function", cache: bool = True, lazy: bool = True):
    """
    参数生成器装饰器，支持两种用法:
    ...
    def decorator(test_func: Callable) -> Callable:
        ...
        for param_name, generator_func in param_mapping.items():
            if not callable(generator_func):
                raise ValueError(f"Generator for parameter '{param_name}' must be callable")
            ...
            if not hasattr(generator_func, '_is_param_generator'):
                ...
                raise ValueError(f"Function {func_name} must be decorated with @param_generator")
