    ...
    def validate(self) -> bool:
        ...
        try:
            ...
            if not isinstance(self.get("cache", "enabled"), bool):
                raise ConfigurationError("cache.enabled", self.get("cache", "enabled"), bool)
            
            # 验证验证级别
            valid_levels = ["strict", "warn", "off"]
            if self.get("validation", "level") not in valid_levels:
                raise ConfigurationError("validation.level", self.get("validation", "level"), str)
            
            # 验证日志级别
            valid_log_levels = ["DEBUG", "INFO", "WARNING", "ERROR"]
            if self.get("validation", "log_level") not in valid_log_levels:
                raise ConfigurationError("validation.log_level", self.get("validation", "log_level"), str)
