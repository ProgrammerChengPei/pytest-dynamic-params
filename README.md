...
1. **动态数据生成受限**：
传统 `@pytest.mark.parametrize` 在模块导入阶段执行，而 fixture 在测试运行阶段才实例化，导致无法在参数化中使用 fixture 的值，限制了动态数据生成的能力
...
- **fixture参数化**：支持动态生成fixture值，fixture可以动态参数，实现fixture的参数化
- **嵌套关系支持**：支持静态参数、动态参数、静态fixture、动态fixture之间的各种嵌套调用关系
...
- **fixture参数化**：通过参数生成器动态生成fixture值，实现fixture的参数化和依赖管理

## 如何解决核心问题

### 1. 动态数据生成受限

**问题**：传统 `@pytest.mark.parametrize` 在模块导入阶段执行，而 fixture 在测试运行阶段才实例化，导致无法在参数化中使用 fixture 的值，限制了动态数据生成的能力。
...
@dynamic_params(
    ...
    derived_value=generate_derived_value
)
@pytest.mark.parametrize("input_value", [1, 2, 3])
def test_nested_relationships(
    input_value, environment, result, env_config, test_data, derived_value
):
    assert result == input_value * 2
    assert env_config == f"config_{environment}"
    assert test_data["app_name"] == "test_app"
    assert derived_value == result * 10
```
...
- 静态 fixture 调用静态参数
- 静态 fixture 调用动态参数
- 静态 fixture 调用静态 fixture
- 静态 fixture 调用动态 fixture
- 动态 fixture 调用静态参数
- 动态 fixture 调用动态参数
- 动态 fixture 调用静态 fixture
- 动态 fixture 调用动态 fixture
