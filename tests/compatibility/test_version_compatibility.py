...
class TestDynamicParamsCoreCompatibility:
    ...
    def test_dynref_basic(self):
        ...
        # Test that DynRef has proper attributes
        assert hasattr(expr, 'resolve') or hasattr(expr, '_left') or hasattr(expr, '_right')
...
class TestDecoratorCompatibility:
    ...
    def test_decorator_order(self):
        ...
        @parametrize_test("x", [1, 2])
        def test_func1(x):
            assert x > 0
        
        @parametrize_test("y", [3, 4])
        def test_func2(y):
            assert y > 0
...
class TestImportExportCompatibility:
    ...
    def test_import_engine_components(self):
        ...
        from dynamic_params.engine.dependency import DependencyResolver, DependencyGraph, DynRef
        from dynamic_params.engine.generator import GeneratorBase, GeneratorRegistry, GeneratorCache
        from dynamic_params.engine.parametrize import ParametrizeProcessor, ParametrizeCombinator
