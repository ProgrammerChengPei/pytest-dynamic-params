...
""")
    ...
    def test_xdist_with_dynref(self, isolated_test_env, run_pytest_cmd):
        """Test DynRef compatibility with parallel execution."""
        test_file = isolated_test_env / "test_dynref_xdist.py"
        test_file.write_text("""
import pytest
...
def test_dynref_parallel(a, b, expected):
    '''Test DynRef with parallel execution'''
    assert a + b == expected
...
@parametrize_test("value", generate_expensive_data)
...
@parametrize_test("case_id", generate_test_cases)
...
@parametrize_test("base", generate_base_values)
...
def test_dynref_load_balance(x, y, sum):
    '''Test DynRef with load balancing'''
    assert x + y == sum
""")
...
@parametrize_generator("base_value", generate_base)
@param_generator
...
def test_single_worker_dynref(a, b):
    '''Test DynRef with single worker'''
    assert a < b
""")
