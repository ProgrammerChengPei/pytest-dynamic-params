...
def validate_performance_threshold(execution_time: float, threshold: float, test_name: str) -> None:
    """
    验证性能是否在阈值内
    ...
    """
    assert execution_time < threshold, f"{test_name}执行时间过长: {execution_time:.4f}秒"
...
class PerformanceReport:
    ...
    def add_result(self, test_name: str, execution_time: float, memory_usage: float = None):
        """
        添加测试结果
