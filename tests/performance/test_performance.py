...
from tests.utils import measure_execution_time, run_with_gc, generate_test_data, validate_performance_threshold, TestClass
...
class TestPerformance:
    ...
    def test_without_cache_performance_comparison(self):
        ...
        assert call_count == expected_calls, f"非缓存版本调用次数异常: 实际{call_count}次，预期{expected_calls}次"
        # 验证所有结果正确
    ...
    def test_nested_generators_performance(self):
        ...
        @with_dynamic_params(
            ...
            transformed_value=generate_transformed
        )
    ...
    def test_memory_usage_measurement(self):
        """测试内存使用情况"""
        from tests.performance.utils import measure_memory_usage, get_current_memory_usage
