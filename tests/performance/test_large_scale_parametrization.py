...
class TestLargeParameterSets:
    ...
    def test_large_parameter_values(self, timer):
        ...
        @pytest.mark.parametrize("data", param_values)
        def test_func(data):
            return len(data)
        execution_times = []
        for i in range(100):
...
class TestCombinatorialParametrization:
    ...
    def test_cartesian_product_medium(self, timer):
        ...
        @pytest.mark.parametrize("x", [[i] for i in range(20)])
        @pytest.mark.parametrize("y", [[i] for i in range(20)])
        @pytest.mark.parametrize("z", [[i] for i in range(5)])
        def test_func(x, y, z):
            return x + y + z
        # Test subset
        test_count = 0
...
class TestStressParametrization:
    ...
    def test_stress_parameter_combinations(self, timer, benchmark_config):
        ...
        def generate_params(n):
            return list(range(n))
        @pytest.mark.parametrize("x", generate_params(50))
        @pytest.mark.parametrize("y", generate_params(50))
        def test_func(x, y):
            return x + y
        execution_times = []
        test_count = 0
...
class TestEdgeCases:
    ...
    def test_single_large_parameter(self, timer):
        ...
        @pytest.mark.parametrize("data", [[large_data]])
        def test_func(data):
            return len(data)
        with timer:
